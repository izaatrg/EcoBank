...
<div class="space-y-6">
    <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        <div class="space-y-2">
            <h1 class="text-2xl font-semibold text-slate-900">Edit Petugas</h1>
            <p class="text-sm text-slate-500">Perbarui informasi petugas agar data tetap akurat dan terpusat.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.petugas.update', $petugas->id) }}" class="mt-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-6 sm:grid-cols-2">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $petugas->name) }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm outline-none transition focus:border-[#1b513e] focus:ring-2 focus:ring-[#1b513e]/20" required>
                </div>

                <div class="space-y-2">
                    <label class="block text-sm font-semibold text-slate-700">Email</label>
                    <input type="email" name="email" value="{{ old('email', $petugas->email) }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm outline-none transition focus:border-[#1b513e] focus:ring-2 focus:ring-[#1b513e]/20" required>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3 pt-4">
                <a href="{{ route('admin.petugas.index') }}" class="rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">Kembali</a>
