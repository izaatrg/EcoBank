...
<head>
    ...
    <style>
        ...
        /* Style warna hijau pastel soft sesuai Figma */
        .sidebar-active {
    ...
    <div class="flex min-h-screen">
        <aside class="w-64 bg-[#E8F9EE] flex flex-col fixed h-full z-50 border-r border-emerald-100/30">
            ...
            <div class="p-4 m-4 bg-emerald-50 rounded-2xl border border-emerald-100/50">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center gap-2 py-2 bg-white hover:bg-red-50 text-red-600 rounded-xl text-xs font-bold transition-all shadow-sm cursor-pointer border border-red-50">
        ...
        <main class="flex-1 ml-64 p-10">
            <header class="flex justify-between items-center mb-8">
                <div>
                    <h2 class="text-xs font-bold text-slate-400 uppercase tracking-widest">Ringkasan Operasional</h2>
                </div>
                <div class="flex items-center gap-4">
                    <div class="relative">
                        <i class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                        <input type="text" placeholder="Cari transaksi..." class="bg-[#E8F9EE]/60 border border-transparent py-2 pl-10 pr-4 rounded-xl text-xs outline-none focus:bg-white focus:border-emerald-500 transition-all w-64">
                    </div>
                    <button class="w-9 h-9 flex items-center justify-center bg-white border border-slate-100 rounded-xl text-slate-500 hover:text-emerald-600 shadow-sm transition-all">
