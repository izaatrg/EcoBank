...
<div class="bg-white p-8 rounded-lg shadow max-w-2xl">
    <form action="{{ route('petugas.profile.update') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" value="{{ $petugas->name }}" class="w-full px-4 py-2 border rounded" required>
        </div>

        <div class="mb-4">
            <label class="block font-semibold mb-2">Email</label>
            <input type="email" name="email" value="{{ $petugas->email }}" class="w-full px-4 py-2 border rounded" required>
        </div>

        <div class="mb-4">
            <label class="block font-semibold mb-2">No. HP</label>
            <input type="text" name="no_hp" value="{{ $petugas->no_hp }}" class="w-full px-4 py-2 border rounded">
        </div>

        <div class="mb-6">
            <label class="block font-semibold mb-2">Alamat</label>
            <textarea name="alamat" class="w-full px-4 py-2 border rounded" rows="4">{{ $petugas->alamat }}</textarea>
        </div>
        ...
        <div class="flex gap-2 mt-6">
            ...
            <a href="{{ route('petugas.dashboard') }}" class="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">Batal</a>
        </div>
    </form>
</div>
